/*******************************************************
* UFXCTPTradeApiSimpleDemoVC2015
* www.xfinapi.com
*******************************************************/
...
2.拷贝XTA_W32\Api\UFXCTP_vX.X.X.X目录（X.X.X.X为版本号）下的所有dll文件到可执行文件目录，
...
{
	printf("  Tick, %s, HighestPrice=%g, LowestPrice=%g, BidPrice0=%g, BidVolume0=%lld, AskPrice0=%g, AskVolume0=%lld, LastPrice=%g, TotalVolume=%lld, TradingTime=%s\n",
...
{
	printf("  Ref=%s, ID=%s, InstID=%s, Price=%g, Volume=%lld, NoTradedVolume=%lld, Direction=%d, PriceCond=%d, SessionID=%lld, Status=%d, Msg=%s, %s\n",
...
{
	printf("  ID=%s, OrderRef=%s, InstID=%s, Price=%g, Volume=%lld, Direction=%d, %s\n",
...
{
	printf("  InstID=%s, Direction=%d, PositionTotal=%lld, PositionSellable=%lld, PosYesterday=%lld, AvgPrice=%g\n",
...
void MarketTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	market = XFinApi_CreateMarketApi("XTA_W32/Api/UFXCTP_V3.7.1.10/XFinApi.UFXCTPTradeApid.dll", &err);
#else
	market = XFinApi_CreateMarketApi("XTA_W32/Api/UFXCTP_V3.7.1.10/XFinApi.UFXCTPTradeApi.dll", &err);
#endif
...
void TradeTest()
{
	...
	int err = -1;
#ifdef _DEBUG
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/UFXCTP_V3.7.1.10/XFinApi.UFXCTPTradeApid.dll", &err);
#else
	trade = XFinApi_CreateTradeApi("XTA_W32/Api/UFXCTP_V3.7.1.10/XFinApi.UFXCTPTradeApi.dll", &err);
#endif
	if (err || !trade)
	{
